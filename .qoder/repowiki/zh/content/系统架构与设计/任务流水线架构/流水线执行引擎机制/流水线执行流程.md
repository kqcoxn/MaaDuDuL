...
**本节来源**
- [tasker.py](file://agent/customs/maahelper/tasker.py#L138-L151)
...
**本节来源**
- [tasker.py](file://agent/customs/maahelper/tasker.py#L128-L136)
- [3.1-任务流水线协议.md](file://instructions/maafw-guide/3.1-任务流水线协议.md#顺序检测)

### JumpBack机制

JumpBack功能允许在识别流程中实现复杂的跳转逻辑。当节点配置中包含`jump_back: true`时，系统会在识别到该节点后执行回跳操作，重新定位到指定的起始节点。
...
**本节来源**
- [2.3-回调协议.md](file://instructions/maafw-guide/2.3-回调协议.md#节点识别消息)

### OCR识别在执行流程中的作用

OCR识别在流水线执行中扮演着关键角色，主要用于：

- **文本匹配**: 通过`expected`参数指定期望的文本内容进行匹配
- **状态检测**: 检测界面状态，如"可领取"、"不可领取"等
- **导航定位**: 识别界面元素进行导航操作
...
**本节来源**
- [领取奖励.json](file://assets/resource/base/pipeline/日常任务/领取奖励.json#L138-L144)
...
重复判断 --> 后处理阶段 : repeat = 1
后处理阶段 --> 节点识别
节点识别 --> JumpBack处理 : jump_back = true
JumpBack处理 --> 预处理阶段
节点识别 --> 下一节点 : 识别成功
节点识别 --> 错误处理 : 识别失败
错误处理 --> [*]
...
- [default_pipeline.json](file://assets/resource/base/default_pipeline.json#L2-L6)
...
**本节来源**
- [领取奖励.json](file://assets/resource/base/pipeline/日常任务/领取奖励.json#L311-L332)
...
**本节来源**
- [回到主界面.json](file://assets/resource/base/pipeline/通用/回到主界面.json#L91-L111)
...
6. **JumpBack**: 仅在必要时使用，避免过度复杂的跳转逻辑

...
- [Changelog.md](file://MFAAvalonia/Resource/Changelog.md)
- [更新记录.md](file://MFAAvalonia/Resource/Announcement/90.更新记录.md)
...
## 更新摘要
**所做更改**   
- 更新了版本信息，反映从v0.2.2升级到v0.2.4
- 新增了清紫糖流水线配置的详细分析，从880行扩展到1255行
- 增强了识别节点和导航逻辑的说明
- 更新了基础设施改进章节，包括Python报告代理版本到0.2.4的升级
- 新增了金币大作战21-24关的适配说明

## 目录
1. [引言](#引言)
2. [版本信息与基础设施改进](#版本信息与基础设施改进)
3. [任务流水线协议规范](#任务流水线协议规范)
4. [核心执行逻辑分析](#核心执行逻辑分析)
5. [工具函数支持机制](#工具函数支持机制)
6. [实际配置文件解析](#实际配置文件解析)
7. [状态转换图](#状态转换图)
...
**更新** 本次更新反映了版本升级到v0.2.4，包括清紫糖流水线配置的重大扩展（从880行到1255行）、新增大量识别节点和导航逻辑、Python报告代理版本更新到0.2.4、界面元数据刷新、版本跟踪和部署流程改进等基础设施改进。

## 版本信息与基础设施改进

### 当前版本状态

根据最新的版本信息，MaaDuDuL当前版本为v0.2.4，该版本包含了多项重要改进和修复：
...
Python报告代理版本已更新到0.2.4，该版本增强了数据上报功能：
...
界面元数据已刷新到v0.2.4版本，包含了完整的界面配置信息：

```json
{
    "interface_version": 2,
    "name": "MaaDuDuL",
    "title": "MDDL 嘟嘟脸小助手(kqcoxn/MaaDuDuL) v0.2.4 - MFAA | 游戏版本：电子羊的永恒之梦",
    "version": "v0.2.4",
    "welcome": "欢迎使用 MDDL!",
...
**Section sources**
- [assets/interface.json:1-85](file://assets/interface.json#L1-L85)

### 版本跟踪机制

版本跟踪机制已完善，支持详细的版本历史记录和更新日志：
...
**Section sources**
- [MFAAvalonia/Resource/Changelog.md:1-25](file://MFAAvalonia/Resource/Changelog.md#L1-L25)

### 部署流程改进

部署流程已进行多项改进，包括嵌入式Python环境的自动化安装：
...
**Section sources**
- [ci/setup_embed_python.ps1:1-136](file://ci/setup_embed_python.ps1#L1-L136)
...
{
    "scripts": {
        "release": "git tag -a v0.2.4 -m \"release v0.2.4\" && git push origin v0.2.4"
    }
}
```
...
**Section sources**
- [3.1-任务流水线协议.md:1-800](file://instructions/maafw-guide/3.1-任务流水线协议.md#L1-L800)
...
**Diagram sources**
- [3.1-任务流水线协议.md:236-255](file://instructions/maafw-guide/3.1-任务流水线协议.md#L236-L255)
...
**Section sources**
- [3.1-任务流水线协议.md:132-255](file://instructions/maafw-guide/3.1-任务流水线协议.md#L132-L255)
...
**Section sources**
- [pipeline_helper.py:1-25](file://agent/customs/global_func/pipeline_helper.py#L1-L25)
...
清紫糖任务配置文件从原来的880行扩展到了1255行，展现了复杂的游戏自动化流程：
...
该任务流程包括任务开始、周期检查、进入冒险界面、进入副本界面、选择关卡、速战等多个复杂步骤，通过`Custom`动作调用周期检查和记录功能，体现了任务的模块化设计。

**更新** 清紫糖配置文件新增了大量识别节点和导航逻辑，包括：
- 金币大作战区域的完整识别系统（B1、B2入口检测）
- 多个滑动操作节点（右上角、右下角、左上角、左下角）
- 详细的副本计数和关卡查找机制
- 完整的导航回退系统

#### 金币大作战识别系统

新增的金币大作战识别系统包含多个专门的识别节点：
...
- [速战.json:1-307](file://assets/resource/base/pipeline/通用/速战.json#L1-L307)
...
MaaDuDuL的任务流水线架构设计体现了高度的模块化和灵活性。通过JSON格式的协议定义，实现了任务逻辑的清晰表达；通过丰富的识别算法和动作类型，支持复杂的游戏自动化操作；通过工具函数的封装，提高了代码的可维护性和可扩展性。实际应用中的配置文件展示了该架构在日常任务和复杂战斗场景中的有效性，而通用模块的设计则体现了良好的可复用性原则。

**更新** 本次版本升级到v0.2.4带来了多项基础设施改进，包括清紫糖流水线配置的重大扩展（从880行到1255行）、新增大量识别节点和导航逻辑、Python报告代理版本更新、界面元数据刷新、版本跟踪机制完善和部署流程优化。清紫糖任务的扩展展现了架构在处理复杂游戏流程方面的强大能力，新增的金币大作战识别系统和智能滑动导航进一步提升了自动化程度。这些改进确保了系统的稳定性和可维护性，为用户提供更好的使用体验。整体架构设计合理，为游戏自动化提供了强大而灵活的解决方案。
# 自定义操作逻辑实现

<cite>
**本文档引用文件**  
- [activity.py](file://agent/customs/special_treat/activity.py)
- [store.py](file://agent/customs/special_treat/store.py)
- [eat_sugar.py](file://agent/customs/special_treat/eat_sugar.py)
- [land_reclamation.py](file://agent/customs/special_treat/land_reclamation.py)
- [tasker.py](file://agent/customs/maahelper/tasker.py)
- [argv_analyzer.py](file://agent/customs/maahelper/argv_analyzer.py)
- [prompter.py](file://agent/customs/utils/prompter.py)
- [matrix_operator.py](file://agent/customs/utils/matrix_operator.py)
</cite>

## 目录
1. [项目结构概述](#项目结构概述)
2. [核心组件分析](#核心组件分析)
3. [自定义操作函数实现模式](#自定义操作函数实现模式)
4. [参数解析机制](#参数解析机制)
5. [基础操作封装](#基础操作封装)
6. [条件判断与循环重试](#条件判断与循环重试)
7. [错误处理与日志记录](#错误处理与日志记录)
8. [矩阵位置计算工具](#矩阵位置计算工具)
9. [MaaFramework集成接口](#maaframework集成接口)
10. [新增功能：金币大作战关卡选择](#新增功能金币大作战关卡选择)

## 项目结构概述

本项目采用模块化设计，核心自定义操作逻辑位于 `agent/customs/special_treat/` 目录下。主要功能模块包括：
- `activity.py`：处理活动界面导航和糖果领取等任务
- `store.py`：实现商店购买和礼包领取功能
- `eat_sugar.py`：提供关卡选择功能，包括克隆工厂、到手蜡和金币大作战关卡选择
- `land_reclamation.py`：实现开荒功能，包括新商品查看与领取
- `maahelper/`：提供任务执行器、参数解析器等辅助工具
- `utils/`：包含提示器、矩阵计算器等通用工具

系统通过MaaFramework的自定义操作机制，将复杂的自动化任务分解为可复用的自定义动作单元。

**Section sources**
- [activity.py](file://agent/customs/special_treat/activity.py)
- [store.py](file://agent/customs/special_treat/store.py)
- [eat_sugar.py](file://agent/customs/special_treat/eat_sugar.py)
- [land_reclamation.py](file://agent/customs/special_treat/land_reclamation.py)

## 核心组件分析

项目的核心是基于MaaFramework构建的自定义操作器，通过继承`CustomAction`类并使用`@AgentServer.custom_action`装饰器注册具体操作。每个自定义动作类都实现了`run`方法，接收上下文和参数对象作为输入。

关键组件包括：
- **EnterActivity**：通过活动标题进入指定活动界面
- **ClaimCandy**：根据时间段领取糖果奖励
- **Gift**：领取指定名称的礼包
- **Buy**：购买指定商品
- **SelectCloneLevel**：选择克隆工厂关卡（4列布局）
- **SelectCrayonLevel**：选择到手蜡关卡（5列布局）
- **SelectDuplicateLevel**：选择副本关卡（支持格式化关卡编号）
- **SelectGoldLevel**：选择金币大作战关卡（13-20级）

这些组件通过调用`Tasker`类执行预定义的流水线任务来完成具体操作。

```mermaid
classDiagram
class CustomAction {
<<abstract>>
+run(context : Context, argv : RunArg) bool
}
class EnterActivity {
+run(context : Context, argv : RunArg) bool
}
class ClaimCandy {
+run(context : Context, argv : RunArg) bool
}
class Gift {
+run(context : Context, argv : RunArg) bool
}
class Buy {
+run(context : Context, argv : RunArg) bool
}
class SelectCloneLevel {
+run(context : Context, argv : RunArg) bool
}
class SelectCrayonLevel {
+run(context : Context, argv : RunArg) bool
}
class SelectDuplicateLevel {
+run(context : Context, argv : RunArg) bool
}
class SelectGoldLevel {
+run(context : Context, argv : RunArg) bool
}
CustomAction <|-- EnterActivity
CustomAction <|-- ClaimCandy
CustomAction <|-- Gift
CustomAction <|-- Buy
CustomAction <|-- SelectCloneLevel
CustomAction <|-- SelectCrayonLevel
CustomAction <|-- SelectDuplicateLevel
CustomAction <|-- SelectGoldLevel
```

**Diagram sources**
- [activity.py:17-55](file://agent/customs/special_treat/activity.py#L17-L55)
- [store.py:14-96](file://agent/customs/special_treat/store.py#L14-L96)
- [eat_sugar.py:65-237](file://agent/customs/special_treat/eat_sugar.py#L65-L237)

**Section sources**
- [activity.py:1-145](file://agent/customs/special_treat/activity.py#L1-L145)
- [store.py:1-96](file://agent/customs/special_treat/store.py#L1-L96)
- [eat_sugar.py:1-238](file://agent/customs/special_treat/eat_sugar.py#L1-L238)

## 自定义操作函数实现模式

自定义操作函数遵循统一的实现模式，以`activity.py`中的`EnterActivity`类为例：

```mermaid
sequenceDiagram
participant User as "用户"
participant Action as "自定义动作"
participant Tasker as "Tasker"
participant Pipeline as "流水线任务"
User->>Action : 触发自定义动作
Action->>Action : 解析输入参数
Action->>Tasker : 调用run方法
Tasker->>Pipeline : 执行流水线任务
Pipeline-->>Tasker : 返回执行结果
Tasker-->>Action : 返回任务详情
Action->>Action : 检查执行结果
Action-->>User : 返回布尔结果
```

**Diagram sources**
- [activity.py:24-55](file://agent/customs/special_treat/activity.py#L24-L55)
- [tasker.py:51-113](file://agent/customs/maahelper/tasker.py#L51-L113)

**Section sources**
- [activity.py:17-55](file://agent/customs/special_treat/activity.py#L17-L55)

## 参数解析机制

系统通过`ParamAnalyzer`类实现灵活的参数解析机制，支持多种参数格式：

```mermaid
flowchart TD
Start([开始解析参数]) --> ParseType["判断参数类型"]
ParseType --> JSONCheck{"是否为JSON格式?"}
JSONCheck --> |是| ParseJSON["解析JSON字符串"]
JSONCheck --> |否| QueryCheck{"是否包含'='?"}
QueryCheck --> |是| ParseQuery["解析查询字符串"]
QueryCheck --> |否| ReturnEmpty["返回空字典"]
ParseJSON --> ConvertNumber["转换数字类型"]
ParseQuery --> ConvertNumber
ConvertNumber --> End([返回解析结果])
```

参数解析支持以下特性：
- 多种格式：JSON对象、JSON数组、查询字符串
- 别名支持：允许为同一参数定义多个名称
- 类型转换：自动将数字字符串转换为int或float类型
- 默认值：可指定默认返回值

**Section sources**
- [argv_analyzer.py:17-159](file://agent/customs/maahelper/argv_analyzer.py#L17-L159)

## 基础操作封装

通过`Tasker`类封装了基础的设备控制操作，提供了简洁的链式调用接口：

```mermaid
classDiagram
class Tasker {
+context : Context
+ctl : Controller
+screenshot() np.ndarray
+click(x : int, y : int) Tasker
+swipe(x1 : int, y1 : int, x2 : int, y2 : int, duration : int) Tasker
+wait(minutes : float) Tasker
+run(entry : str, pipeline_override : Dict) TaskDetail
}
class Controller {
+post_screencap() Job
+post_click(x : int, y : int) Job
+post_swipe(x1 : int, y1 : int, x2 : int, y2 : int, duration : int) Job
}
Tasker --> Controller : "通过context.tasker.controller访问"
```

**Diagram sources**
- [tasker.py:16-177](file://agent/customs/maahelper/tasker.py#L16-L177)

**Section sources**
- [tasker.py:16-177](file://agent/customs/maahelper/tasker.py#L16-L177)

## 条件判断与循环重试

系统通过流水线配置实现条件判断和循环重试逻辑。以`claim_candy`为例，其执行流程如下：

```mermaid
flowchart TD
Start([开始领取糖果]) --> CheckPeriod["周期检查"]
CheckPeriod --> |已领取| RecordPeriod["记录周期"]
CheckPeriod --> |未领取| ClickClaim["点击领取"]
ClickClaim --> CheckReceived["检查已领取"]
CheckReceived --> |成功| RecordPeriod
CheckReceived --> |失败| HandleError["处理错误"]
RecordPeriod --> End([结束])
HandleError --> End
```

这种模式通过MaaFramework的流水线机制实现，无需在代码中显式编写循环逻辑。

**Section sources**
- [activity.py:60-102](file://agent/customs/special_treat/activity.py#L60-L102)
- [assets/resource/base/pipeline/日常任务/领取糖果.json](file://assets/resource/base/pipeline/日常任务/领取糖果.json)

## 错误处理与日志记录

系统采用统一的错误处理和日志记录机制：

```mermaid
sequenceDiagram
participant Action as "自定义动作"
participant Prompter as "Prompter"
participant Console as "控制台"
Action->>Action : 执行业务逻辑
Action->>Prompter : 调用log方法记录信息
Prompter->>Console : 输出带前缀的日志
Action->>Action : 捕获异常
Action->>Prompter : 调用error方法处理错误
Prompter->>Console : 输出错误分隔线
Prompter->>Console : 输出错误详情
Prompter->>Console : 输出错误分隔线
Prompter-->>Action : 返回False
Action-->>调用者 : 返回False
```

**Diagram sources**
- [prompter.py:16-55](file://agent/customs/utils/prompter.py#L16-L55)

**Section sources**
- [prompter.py:16-55](file://agent/customs/utils/prompter.py#L16-L55)

## 矩阵位置计算工具

`MatrixOperator`类提供了矩阵布局中元素坐标的计算功能：

```mermaid
classDiagram
class MatrixOperator {
-start_pos : tuple[int, int]
-step : tuple[int, int]
-width : int
-height : int
+__init__(origin_x : int, origin_y : int, step_x : int, step_y : int, width : int, height : int)
+get_pos(row : int, column : int) tuple[int, int]
}
```

该工具支持：
- 正向索引：从1开始计算位置
- 反向索引：负数表示从末尾倒数
- 边界检查：确保索引在有效范围内

**Diagram sources**
- [matrix_operator.py:1-58](file://agent/customs/utils/matrix_operator.py#L1-L58)

**Section sources**
- [matrix_operator.py:1-58](file://agent/customs/utils/matrix_operator.py#L1-L58)

## MaaFramework集成接口

系统遵循MaaFramework的标准化接口设计规范：

```mermaid
erDiagram
CUSTOM_ACTION {
string name PK
string description
string param_format
string implementation_file
}
PIPELINE_TASK {
string entry_node PK
json override_config
string description
string source_file
}
PARAMETER {
string key PK
string alias
string type
string default_value
string description
}
CUSTOM_ACTION ||--o{ PIPELINE_TASK : "可被调用"
CUSTOM_ACTION ||--o{ PARAMETER : "接受参数"
```

**Section sources**
- [instructions/maafw-guide/4.2-标准化接口设计.md:12-46](file://instructions/maafw-guide/4.2-标准化接口设计.md#L12-L46)
- [instructions/maafw-guide/2.2-集成接口一览.md:1-401](file://instructions/maafw-guide/2.2-集成接口一览.md#L1-L401)

## 新增功能：金币大作战关卡选择

**更新** 新增了专门针对金币大作战关卡选择的自定义动作，支持13-20级关卡的智能导航和坐标计算。

### SelectGoldLevel自定义动作实现

`SelectGoldLevel`类实现了金币大作战关卡选择功能，具有以下特点：

#### 功能特性
- **关卡范围限制**：仅支持13-20级关卡的选择
- **智能导航**：根据关卡编号自动选择右上角或右下角导航按钮
- **坐标计算**：通过`MatrixOperator`类计算精确的点击坐标
- **边界条件处理**：对13-20级关卡进行特殊处理

#### 实现细节

```mermaid
flowchart TD
Start([开始选择金币关卡]) --> ParseParam["解析关卡参数"]
ParseParam --> TypeCheck{"参数类型检查"}
TypeCheck --> |非整数| ConvertInt["转换为整数"]
TypeCheck --> |整数| RangeCheck["范围检查"]
ConvertInt --> RangeCheck
RangeCheck --> |< 13| Error["输出错误信息"]
RangeCheck --> |>= 13 & < 19| NavigateUp["导航至右上角"]
RangeCheck --> |>= 19| NavigateDown["导航至右下角"]
NavigateUp --> ClickLevel["点击目标关卡"]
NavigateDown --> ClickLevel
Error --> End([结束])
ClickLevel --> End
```

#### 参数处理
- **参数名称**：`level` 或 `l`
- **数据类型**：整数型
- **有效范围**：13-20
- **默认行为**：自动转换字符串类型的关卡编号

#### 智能导航逻辑
- **13-18级关卡**：点击右上角导航按钮
- **19-20级关卡**：点击右下角导航按钮
- **坐标计算**：使用`MatrixOperator`类进行精确坐标定位

#### 错误处理
- **无效关卡**：当关卡编号小于13时，返回错误信息
- **类型转换**：自动将字符串转换为整数
- **异常捕获**：统一的异常处理机制

**Section sources**
- [eat_sugar.py:209-237](file://agent/customs/special_treat/eat_sugar.py#L209-L237)
...
## 简介
MaaDuDuL的任务流水线（Pipeline）系统是一个基于JSON配置的自动化任务执行框架，用于定义和执行游戏自动化任务。该系统通过模块化的JSON配置文件来描述任务流程，支持复杂的条件跳转、超时处理、自定义逻辑以及增强的错误处理与恢复策略。本文档将深入解析该系统的配置机制，以"日常任务"中的"巅峰对决.json"和"领取邮件.json"为例，详细说明其核心概念和实现原理，并重点介绍新增的JumpBack机制、错误处理增强与异常恢复策略。
...
### JumpBack机制
JumpBack机制允许节点在执行过程中回跳到之前定义的锚点或特定节点，实现灵活的流程控制。在"回到主界面.json"中，可以看到JumpBack的典型用法：
...
### 进程守护器
`process_guard.py`提供了任务停止状态检测功能：

```python
@AgentServer.custom_recognition("process_guard")
class ProcessGuard(CustomRecognition):
    def analyze(self, context: Context, argv: CustomRecognition.AnalyzeArg) -> CustomRecognition.AnalyzeResult:
        try:

...
**Section sources**
- [2.3-回调协议.md](file://instructions/maafw-guide/2.3-回调协议.md#L1-L19)
...
**Section sources**
- [2.3-回调协议.md](file://instructions/maafw-guide/2.3-回调协议.md#L58-L92)
...
**Section sources**
- [2.3-回调协议.md](file://instructions/maafw-guide/2.3-回调协议.md#L93-L124)
...
**Section sources**
- [2.3-回调协议.md](file://instructions/maafw-guide/2.3-回调协议.md#L176-L207)
...
**Section sources**
- [2.3-回调协议.md](file://instructions/maafw-guide/2.3-回调协议.md#L208-L239)
...
**Section sources**
- [2.3-回调协议.md](file://instructions/maafw-guide/2.3-回调协议.md#L241-L272)
...
**Section sources**
- [2.3-回调协议.md](file://instructions/maafw-guide/2.3-回调协议.md#L273-L304)
...
**Section sources**
- [2.3-回调协议.md](file://instructions/maafw-guide/2.3-回调协议.md#L305-L336)
...
**Section sources**
- [2.3-回调协议.md](file://instructions/maafw-guide/2.3-回调协议.md#L337-L357)

## 调试技巧

利用回调信息可以有效监控执行流程和诊断识别失败原因：

1. **执行流程监控**：通过监听 `Tasker.Task.Starting` 和 `Tasker.Task.Succeeded` 消息，可以跟踪任务的开始和结束时间，分析任务执行效率。
2. **识别失败诊断**：当收到 `Node.Recognition.Failed` 消息时，结合 `details_json` 中的节点信息，可以定位具体是哪个节点识别失败，进而检查该节点的图像匹配设置或调整识别参数。
3. **资源加载验证**：通过 `Resource.Loading.Starting` 和 `Resource.Loading.Succeeded` 消息，可以确认资源是否正确加载，避免因资源缺失导致的运行异常。
4. **控制器动作验证**：监听 `Controller.Action.Starting` 和 `Controller.Action.Succeeded` 消息，可以验证点击、滑动等操作是否成功执行。

# 特殊处理模块实现分析

<cite>
**本文档引用文件**  
- [activity.py](file://agent/customs/special_treat/activity.py)
- [peak_showdown.py](file://agent/customs/special_treat/peak_showdown.py)
- [receive_reward.py](file://agent/customs/special_treat/receive_reward.py)
- [store.py](file://agent/customs/special_treat/store.py)
- [eat_sugar.py](file://agent/customs/special_treat/eat_sugar.py)
- [land_reclamation.py](file://agent/customs/special_treat/land_reclamation.py)
- [pinch_face.py](file://agent/customs/special_treat/pinch_face.py)
- [holy.py](file://agent/customs/special_treat/holy.py)
- [exam_cram.py](file://agent/customs/special_treat/exam_cram.py)
- [pipeline_helper.py](file://agent/customs/global_func/pipeline_helper.py)
- [tasker.py](file://agent/customs/maahelper/tasker.py)
- [argv_analyzer.py](file://agent/customs/maahelper/argv_analyzer.py)
- [periodic_check.py](file://agent/customs/global_func/periodic_check.py)
- [local_storage.py](file://agent/customs/utils/local_storage.py)
- [clear_red_candy.json](file://assets/resource/tasks/daily/clear_red_candy.json)
- [clear_red_candy.md](file://assets/resource/descs/daily/clear_red_candy.md)
- [圣团巡礼.json](file://assets/resource/base/pipeline/日常任务/圣团巡礼.json)
- [saint_tour.json](file://assets/resource/tasks/daily/saint_tour.json)
</cite>

## 目录
1. [项目结构](#项目结构)  
2. [核心组件分析](#核心组件分析)  
3. [活动任务触发与状态机设计](#活动任务触发与状态机设计)  
4. [巅峰对决周期检测机制](#巅峰对决周期检测机制)  
5. [奖励领取通用模式抽象](#奖励领取通用模式抽象)  
6. [商店刷新与购买策略](#商店刷新与购买策略)  
7. [轻量级业务逻辑封装](#轻量级业务逻辑封装)  
8. [清红糖模块实现分析](#清红糖模块实现分析)  
9. [每日宴会功能实现分析](#每日宴会功能实现分析)  
10. [主流程集成机制](#主流程集成机制)  
11. [可复用设计模式建议](#可复用设计模式建议)

## 项目结构

特殊处理模块位于 `agent/customs/special_treat/` 目录下，包含多个独立功能的 Python 文件，每个文件实现特定的游戏自动化逻辑。这些模块通过 MaaFramework 的自定义动作（CustomAction）和自定义识别（CustomRecognition）机制与主流程集成。

```mermaid
graph TD
subgraph "特殊处理模块"
A[activity.py] --> |进入活动/领取糖果| Pipeline
B[peak_showdown.py] --> |选择对手/解析战力| Pipeline
C[receive_reward.py] --> |领取任务/通行证奖励| Pipeline
D[store.py] --> |购买商品/领取礼包| Pipeline
E[eat_sugar.py] --> |关卡选择/快速战斗| Pipeline
F[land_reclamation.py] --> |土地开垦| Pipeline
G[pinch_face.py] --> |捏脸功能| Pipeline
H[clean_red_candy.py] --> |清红糖关卡| Pipeline
I[holy.py] --> |宴席邀请| Pipeline
J[exam_cram.py] --> |短期速成班| Pipeline
end
subgraph "核心辅助模块"
K[pipeline_helper.py] --> |通用任务执行| SpecialTreat
L[tasker.py] --> |任务执行器封装| SpecialTreat
M[argv_analyzer.py] --> |参数解析| SpecialTreat
N[periodic_check.py] --> |周期性检查| SpecialTreat
end
Pipeline --> |任务流程定义| MaaFramework
SpecialTreat --> |调用| Pipeline
```

**图示来源**  
- [activity.py](file://agent/customs/special_treat/activity.py)
- [peak_showdown.py](file://agent/customs/special_treat/peak_showdown.py)
- [holy.py](file://agent/customs/special_treat/holy.py)
- [pipeline_helper.py](file://agent/customs/global_func/pipeline_helper.py)

## 核心组件分析

特殊处理模块采用模块化设计，每个功能独立封装，通过统一的接口与 MaaFramework 交互。核心组件包括：

- **自定义动作类**：继承 `CustomAction`，实现 `run` 方法，用于执行具体操作
- **自定义识别类**：继承 `CustomRecognition`，实现 `analyze` 方法，用于状态判断
- **参数解析器**：`ParamAnalyzer` 统一处理输入参数，支持多种格式解析
- **任务执行器**：`Tasker` 封装底层操作，提供链式调用接口
- **状态存储**：`LocalStorage` 提供持久化存储能力

这些组件共同构成了一个高内聚、低耦合的自动化执行体系。

**本节来源**  
- [tasker.py:1-177](file://agent/customs/maahelper/tasker.py#L1-L177)
- [argv_analyzer.py:1-159](file://agent/customs/maahelper/argv_analyzer.py#L1-L159)
- [local_storage.py:1-111](file://agent/customs/utils/local_storage.py#L1-L111)

## 活动任务触发与状态机设计

以 `activity.py` 为例，该模块实现了活动界面导航和糖果领取功能，其核心是基于状态机的任务编排。

### 活动界面导航

`EnterActivity` 类通过 `Tasker.run()` 方法启动 "进入活动界面_开始" 流程，并传入参数覆盖配置。该流程包含多个节点：
- `进入活动界面_识别活动`：识别指定活动标题
- `进入活动界面_未找到指定活动`：处理未找到活动的情况

执行完成后，通过 `Tasker.get_last_node_name()` 判断最终执行的节点，从而确定是否成功进入目标活动。

### 糖果领取状态机

`ClaimCandy` 类实现了一个多阶段的状态机流程：
1. **周期检查**：通过 `领取糖果_周期检查` 节点判断是否已领取
2. **点击领取**：执行领取操作
3. **状态确认**：检查是否已领取
4. **记录状态**：通过 `领取糖果_周期记录` 更新领取状态

该流程通过 `pipeline_override` 参数动态注入监测逻辑，确保任务执行过程可被监控。

```mermaid
stateDiagram-v2
[*] --> 开始
开始 --> 周期检查
周期检查 --> |可领取| 点击领取
周期检查 --> |已领取| 结束
点击领取 --> 状态确认
状态确认 --> |成功| 记录状态
状态确认 --> |失败| 重试
记录状态 --> 结束
```

**图示来源**  
- [activity.py:59-102](file://agent/customs/special_treat/activity.py#L59-L102)
- [periodic_check.py:1-268](file://agent/customs/global_func/periodic_check.py#L1-L268)

## 巅峰对决周期检测机制

`peak_showdown.py` 模块实现了巅峰对决功能中的对手选择逻辑，其核心是周期性检测与资源消耗控制。

### 战斗力解析

`parse_power()` 函数负责解析 OCR 识别出的战斗力文本。由于 OCR 可能将千分位分隔符识别为逗号或小数点，该函数通过移除所有非数字字符的方式统一处理，确保解析结果的准确性。

### 对手选择策略

`PickOpponent` 类实现了两种选择策略：
- **最低战力**：识别所有对手的战斗力，选择数值最小的目标
- **最高排名**：直接返回预设的最高排名位置

该类通过 `RecoHelper` 进行图像识别，获取所有对手的战斗力文本，然后使用 `min()` 函数配合 `parse_power()` 作为比较键，找到战斗力最低的对手。

### 资源消耗控制

通过 `periodic_check` 机制控制资源消耗，避免重复执行已完成的任务。该机制基于 `LocalStorage` 记录任务完成时间，并根据周期类型（日/周/月）判断是否需要再次执行。

```mermaid
flowchart TD
Start([开始]) --> ParseText["解析战斗力文本"]
ParseText --> RemoveNonDigit["移除非数字字符"]
RemoveNonDigit --> ConvertToInt["转换为整数"]
ConvertToInt --> ReturnResult["返回战斗力数值"]
ReturnResult --> End([结束])
ParseText --> |解析失败| ReturnInf["返回无穷大"]
ReturnInf --> End
```

**图示来源**  
- [peak_showdown.py:17-96](file://agent/customs/special_treat/peak_showdown.py#L17-L96)
- [periodic_check.py:26-253](file://agent/customs/global_func/periodic_check.py#L26-L253)

## 奖励领取通用模式抽象

`receive_reward.py` 模块展示了如何通过抽象实现通用的奖励领取模式。

### 任务奖励领取

`ReceiveTaskReward` 类处理日常任务奖励的领取：
- 接收 `category` 参数（如 "日"、"周"、"月"）
- 自动将 "日" 转换为 "天" 以匹配界面文本
- 启动 "领取奖励_领取任务奖励开始" 流程
- 通过 `expected` 参数指定要进入的具体任务面板

### 通行证奖励领取

`ReceivePassReward` 类处理通行证奖励的领取：
- 接收 `category` 参数（如 "等级"、"便装"、"冒险"）
- 映射到对应的模板文件（如 "grade_pass"、"clothes_path"、"adv_pass"）
- 启动 "领取奖励_领取通行证奖励开始" 流程
- 通过 `template` 参数指定要进入的通行证界面

这种设计体现了**参数驱动**的思想，通过统一的接口处理不同类型的奖励领取，提高了代码的复用性和可维护性。

```mermaid
classDiagram
class ReceiveRewardBase {
+run(context, argv) bool
}
class ReceiveTaskReward {
+run(context, argv) bool
}
class ReceivePassReward {
+run(context, argv) bool
}
ReceiveRewardBase <|-- ReceiveTaskReward
ReceiveRewardBase <|-- ReceivePassReward
```

**图示来源**  
- [receive_reward.py:9-65](file://agent/customs/special_treat/receive_reward.py#L9-L65)
- [activity.py:60-102](file://agent/customs/special_treat/activity.py#L60-L102)

## 商店刷新与购买策略

`store.py` 模块实现了商店相关的购买和礼包领取功能，展示了清晰的决策逻辑。

### 礼包领取策略

`Gift` 类处理礼包领取：
- 接收 `gift` 参数指定礼包名称
- 启动 "每日采购_购买礼包开始" 流程
- 通过 `expected` 参数匹配指定礼包

### 商品购买策略

`Buy` 类处理商品购买：
- 支持多种参数别名（goods/g/commodity/c）
- 启动 "每日采购_购买商品" 流程
- 通过 `expected` 参数匹配指定商品

该模块通过 `ParamAnalyzer.get()` 方法支持参数别名，提高了接口的灵活性。同时，两个类都遵循了**单一职责原则**，每个类只负责一种类型的购买操作。

```mermaid
sequenceDiagram
participant 用户
participant Buy类
participant Tasker
participant Pipeline
用户->>Buy类 : 调用run()方法
Buy类->>Buy类 : 解析参数(goods)
Buy类->>Tasker : run("每日采购_购买商品")
Tasker->>Pipeline : 执行购买流程
Pipeline-->>Tasker : 返回执行结果
Tasker-->>Buy类 : 返回结果
Buy类-->>用户 : 返回True/False
```

**图示来源**  
- [store.py:15-96](file://agent/customs/special_treat/store.py#L15-L96)
- [tasker.py:51-113](file://agent/customs/maahelper/tasker.py#L51-L113)

## 轻量级业务逻辑封装

`eat_sugar.py` 模块展示了如何封装轻量级业务逻辑，以清紫糖关卡选择为例。

### 关卡坐标计算

通过 `MatrixOperator` 类封装坐标计算逻辑：
- 定义起始坐标和间隔
- 根据行列号计算实际屏幕坐标
- 支持不同布局（4列/5列）

### 关卡选择实现

`SelectCloneLevel` 类处理克隆工厂关卡选择：
- 关卡1-12：直接点击可见区域
- 关卡13-20：先向上滑动，再点击对应位置

`SelectCrayonLevel` 类处理到手蜡关卡选择：
- 统一的5列布局
- 自动计算行列位置

`QuickFight` 类实现快速战斗：
- 支持指定作战次数
- 动态覆盖流程配置

这种封装方式将复杂的坐标计算逻辑隐藏在工具类中，使业务逻辑更加清晰易懂。

```mermaid
flowchart TD
Start([开始]) --> ParseLevel["解析关卡编号"]
ParseLevel --> CheckRange["判断关卡范围"]
CheckRange --> |1-12| DirectClick["直接点击"]
CheckRange --> |13-20| SwipeUp["向上滑动"]
SwipeUp --> CalculatePos["计算坐标"]
DirectClick --> CalculatePos
CalculatePos --> ClickPos["执行点击"]
ClickPos --> End([结束])
```

**图示来源**  
- [eat_sugar.py:65-206](file://agent/customs/special_treat/eat_sugar.py#L65-L206)
- [tasker.py:125-156](file://agent/customs/maahelper/tasker.py#L125-L156)

## 清红糖模块实现分析

新增的清红糖模块实现了红色糖果关卡的自动化清理功能，展示了完整的参数化任务执行流程。

### 任务配置与参数验证

清红糖任务通过 JSON 配置文件定义，支持关卡区间的参数化设置：
- **任务入口**：`清红糖_开始`
- **参数选项**：`清红糖-关卡区间`
- **参数格式**：`章-关`（如 "24-9"），支持 1-2 位章号和 1-10 关号
- **默认值**：起始关卡 "1-1"，结束关卡 "1-1"

### 参数管道覆盖机制

通过 `pipeline_override` 实现参数动态注入：
- 在每次任务运行时，将起始关卡和结束关卡参数注入到 "清红糖_指定关卡作战" 节点
- 使用 `sl={起始关卡}&el={结束关卡}` 格式进行参数替换
- 实现了任务执行的动态配置能力

### 关卡区间管理

清红糖模块支持关卡区间的步进循环执行：
- **起始关卡**：从指定章节的指定关卡开始
- **结束关卡**：指定章节的指定关卡结束
- **步进机制**：每次运行时按顺序推进关卡
- **边界处理**：支持跨章节的关卡清理

### 任务描述与界面说明

任务描述文档详细说明了功能用途和界面要求：
- **功能说明**：完成红色糖果关卡的清理任务，获取角色培养所需装备合成素材
- **起止界面**：启动界面为任意界面，结束界面为主界面
- **任务类型**：可连续任务，属于日常任务范畴

```mermaid
flowchart TD
Start([开始]) --> ParseConfig["解析任务配置"]
ParseConfig --> ValidateParams["验证关卡参数"]
ValidateParams --> SetInterval["设置关卡区间"]
SetInterval --> ExecuteTask["执行清红糖任务"]
ExecuteTask --> NextStep["推进到下一关卡"]
NextStep --> CheckEnd["检查是否到达结束关卡"]
CheckEnd --> |未到达| ExecuteTask
CheckEnd --> |到达| End([结束])
```

**图示来源**  
- [clear_red_candy.json:1-38](file://assets/resource/tasks/daily/clear_red_candy.json#L1-L38)
- [clear_red_candy.md:1-13](file://assets/resource/descs/daily/clear_red_candy.md#L1-L13)

**本节来源**  
- [clear_red_candy.json:1-38](file://assets/resource/tasks/daily/clear_red_candy.json#L1-L38)
- [clear_red_candy.md:1-13](file://assets/resource/descs/daily/clear_red_candy.md#L1-L13)

## 每日宴会功能实现分析

新增的每日宴会功能实现了完整的宴席邀请自动化，包括自定义动作开发和管道集成。

### 宴席邀请自定义动作

`holy.py` 文件中的 `Banquet` 类实现了宴席邀请的自定义动作：
- **动作注册**：使用 `@AgentServer.custom_action("banquet")` 装饰器注册
- **参数解析**：支持 `list` 和 `l` 参数别名，接收角色列表
- **空值过滤**：自动过滤空字符串和空白字符
- **逐个邀请**：遍历邀请列表，依次执行邀请操作

### 参数化邀请名单配置

通过 `saint_tour.json` 中的配置实现了邀请名单的参数化：
- **输入字段**：支持5个角色位置的输入框
- **默认值**：每个位置默认为空字符串
- **管道覆盖**：使用 `l={角色1}&l={角色2}&...&l={角色5}` 格式动态生成参数
- **条件开关**：通过 "每日宴会" 开关控制功能启用

### 管道集成与流程控制

宴席功能通过 `圣团巡礼.json` 管道实现完整的工作流：
- **周期检查**：`圣团巡礼_宴会周期检查` 节点控制执行频率
- **进入宴会厅**：`圣团巡礼_进入宴会厅` 节点导航到目标界面
- **开始宴会**：`圣团巡礼_开始宴会` 节点调用 `banquet` 自定义动作
- **周期记录**：`圣团巡礼_宴会周期记录` 节点记录执行状态

### 错误处理与日志记录

系统提供了完善的错误处理机制：
- **异常捕获**：使用 `try-except` 块捕获执行过程中的异常
- **统一错误处理**：通过 `Prompter.error()` 统一处理错误信息
- **日志记录**：使用 `Prompter.log()` 记录邀请过程的关键信息

```mermaid
flowchart TD
Start([开始]) --> ParseArgs["解析邀请参数"]
ParseArgs --> FilterEmpty["过滤空值"]
FilterEmpty --> CheckList["检查邀请列表"]
CheckList --> |有角色| InviteLoop["遍历邀请列表"]
CheckList --> |无角色| ErrorHandle["处理空列表错误"]
InviteLoop --> CallAction["调用banquet动作"]
CallAction --> LogProgress["记录邀请进度"]
LogProgress --> NextCharacter["邀请下一个角色"]
NextCharacter --> MoreCharacters{"还有角色?"}
MoreCharacters --> |是| InviteLoop
MoreCharacters --> |否| Success["邀请完成"]
ErrorHandle --> End([结束])
Success --> End
```

**图示来源**  
- [holy.py:27-58](file://agent/customs/special_treat/holy.py#L27-L58)
- [saint_tour.json:137-178](file://assets/resource/tasks/daily/saint_tour.json#L137-L178)
- [圣团巡礼.json:1078-1097](file://assets/resource/base/pipeline/日常任务/圣团巡礼.json#L1078-L1097)

**本节来源**  
- [holy.py:1-58](file://agent/customs/special_treat/holy.py#L1-L58)
- [saint_tour.json:102-217](file://assets/resource/tasks/daily/saint_tour.json#L102-L217)
- [圣团巡礼.json:1078-1097](file://assets/resource/base/pipeline/日常任务/圣团巡礼.json#L1078-L1097)

## 主流程集成机制

特殊处理模块通过 `pipeline_helper.py` 与主流程集成，形成统一的执行体系。

### 统一执行接口

`pipeline_helper.Run` 类提供统一的 `run` 接口：
- 接收任务入口节点名称
- 支持指定期望的结束节点
- 自动处理异常并返回布尔结果

### 集成方式

各模块通过以下方式与主流程集成：
1. **自定义动作注册**：使用 `@AgentServer.custom_action` 装饰器注册
2. **参数传递**：通过 `argv.custom_action_param` 传递参数
3. **流程调用**：使用 `Tasker.run()` 启动预定义的 Pipeline 流程
4. **状态判断**：通过检查最后执行的节点名称判断结果

这种集成方式实现了**关注点分离**，业务逻辑与流程执行解耦，提高了系统的可维护性和扩展性。

```mermaid
graph TB
subgraph "特殊处理模块"
A[activity.py]
B[peak_showdown.py]
C[receive_reward.py]
D[clean_red_candy.py]
E[holy.py]
end
subgraph "集成层"
F[pipeline_helper.py]
G[Tasker]
end
subgraph "执行层"
H[Pipeline]
I[MaaFramework]
end
A --> F
B --> F
C --> F
D --> F
E --> F
F --> G
G --> H
H --> I
```

**图示来源**  
- [pipeline_helper.py:9-25](file://agent/customs/global_func/pipeline_helper.py#L9-L25)
- [tasker.py:51-113](file://agent/customs/maahelper/tasker.py#L51-L113)

## 可复用设计模式建议

基于对特殊处理模块的分析，提出以下可复用的设计模式：

### 参数驱动模式

通过 `ParamAnalyzer` 统一解析参数，支持多种格式（JSON、查询字符串），并自动转换数据类型。建议在新模块中复用此模式，提高接口的灵活性和易用性。

### 状态机编排模式

将复杂操作分解为多个阶段，通过 Pipeline 定义状态转移逻辑。建议在涉及多步骤操作的场景中使用此模式，如任务领取、战斗流程等。

### 工具类封装模式

将通用逻辑（如坐标计算、参数解析）封装在独立的工具类中，如 `MatrixOperator`、`ParamAnalyzer`。建议在新项目中建立类似的工具库，提高代码复用率。

### 单一职责模式

每个类只负责一个特定功能，如 `Gift` 只处理礼包领取，`Buy` 只处理商品购买，`Banquet` 只处理宴席邀请。建议在设计新类时遵循此原则，保持代码的清晰性和可维护性。

### 异常统一处理模式

所有 `run` 方法都包含 `try-catch` 块，捕获异常并通过 `Prompter.error` 统一处理。建议在所有对外接口中采用此模式，确保系统的稳定性。

### 参数化任务配置模式

通过 JSON 配置文件定义任务参数和验证规则，支持动态参数注入和管道覆盖。建议在需要灵活配置的任务中采用此模式，提高任务的适应性和可维护性。

### 条件开关与管道覆盖模式

通过配置文件中的开关控制功能启用，使用 `pipeline_override` 实现动态参数注入。这种模式特别适用于需要用户自定义输入的功能，如每日宴会的邀请名单配置。

这些设计模式共同构成了一个高效、可靠、易维护的自动化执行框架，值得在后续开发中持续推广和优化。

**本节来源**  
- [argv_analyzer.py:17-159](file://agent/customs/maahelper/argv_analyzer.py#L17-L159)
- [tasker.py:51-113](file://agent/customs/maahelper/tasker.py#L51-L113)
- [pipeline_helper.py:9-25](file://agent/customs/global_func/pipeline_helper.py#L9-L25)
- [saint_tour.json:102-217](file://assets/resource/tasks/daily/saint_tour.json#L102-L217)
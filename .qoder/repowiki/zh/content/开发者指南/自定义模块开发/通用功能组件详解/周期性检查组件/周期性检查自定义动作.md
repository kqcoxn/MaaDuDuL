...
**更新** 新增了对心形糖功能的周期性检查机制集成，心形糖挑战采用了与现有系统相同的模式，包括完整的周期检查、执行和记录流程。
...
**Section sources**
- [periodic_check.py:255-279](file://agent/customs/global_func/periodic_check.py#L255-L279)
...
- `k=心形`: 心形糖挑战，使用默认每日周期检查

**Section sources**
- [清红糖.json:147-189](file://assets/resource/base/pipeline/日常任务/清红糖.json#L147-L189)
- [清紫糖.json:220-262](file://assets/resource/base/pipeline/日常任务/清紫糖.json#L220-L262)
- [每日采购.json:70-269](file://assets/resource/base/pipeline/日常任务/每日采购.json#L70-L269)
...
**Section sources**
- [periodic_check.py:183-279](file://agent/customs/global_func/periodic_check.py#L183-L279)
- [清红糖.json:147-189](file://assets/resource/base/pipeline/日常任务/清红糖.json#L147-L189)
...
**Section sources**
- [local_storage.py:1-111](file://agent/customs/utils/local_storage.py#L1-L111)
- [periodic_check.py:67-82](file://agent/customs/global_func/periodic_check.py#L67-L82)

## 心形糖功能集成

**更新** 心形糖功能是本次更新的主要内容，它完全集成了现有的周期性检查机制。

### 功能概述
心形糖挑战是清红糖任务中的一个可选分支，允许玩家直接进行心形关卡的速刷。该功能采用了与现有周期性检查系统完全相同的模式，确保心形糖挑战的自动执行和状态管理。

### 实现架构
心形糖功能包含三个核心节点：
1. **周期检查节点** (`清红糖_心形周期检查`): 使用`periodic_check`动作检查是否需要执行心形挑战
2. **执行节点** (`清红糖_娃娃怪破坏者`): 实际执行心形关卡的战斗
3. **记录节点** (`清红糖_心形周期记录`): 使用`record_period`动作记录完成时间

### 参数配置
心形糖功能使用`k=心形`作为任务标识符，这与其他周期性任务保持一致：
...
### 与现有系统的兼容性
心形糖功能完全兼容现有的周期性检查系统，包括：
- 使用相同的`Inspector`类进行日期比较
- 采用相同的`_adjust_datetime`方法处理游戏刷新时间
- 使用相同的本地存储机制
- 遵循相同的错误处理和日志记录模式
...
```mermaid
flowchart TD
Start([开始]) --> Check["周期检查: k=心形"]
Check --> NeedExecute{"需要执行?"}
NeedExecute --> |否| Skip["跳过心形挑战"]
NeedExecute --> |是| Execute["执行心形战斗"]
Execute --> Success{"战斗成功?"}
Success --> |是| Record["记录完成时间"]
...
**Section sources**
- [clear_red_candy.json:13-30](file://assets/resource/tasks/daily/clear_red_candy.json#L13-L30)
- [清红糖.json:147-189](file://assets/resource/base/pipeline/日常任务/清红糖.json#L147-L189)
...
**Section sources**
- [continuous_battle.md:9-13](file://assets/resource/descs/single/continuous_battle.md#L9-L13)
...
**Diagram sources**
- [连续作战.json:79-118](file://assets/resource/base/pipeline/开荒功能/连续作战.json#L79-L118)
...
**Section sources**
- [连续作战.json:1-120](file://assets/resource/base/pipeline/开荒功能/连续作战.json#L1-L120)
...
**Diagram sources**
- [process_guard.py:72-99](file://agent/customs/global_func/process_guard.py#L72-L99)
...
- [连续作战.json](file://assets/resource/base/pipeline/开荒功能/连续作战.json#L50)
- [process_guard.py:72-99](file://agent/customs/global_func/process_guard.py#L72-L99)
- [reco_helper.py:232-255](file://agent/customs/maahelper/reco_helper.py#L232-L255)

## 性能优化建议

**更新** 基于最新的每日活动作战管道变更，重点优化OCR准确性和自动战斗识别精度

### ROI参数重算与坐标调整
根据每日活动作战管道的最新变更，时间戳从1772105667532同步到1773304181013，系统进行了全面的ROI参数重算和坐标调整：

1. **OCR识别区域优化**：重新计算了关键界面的OCR识别ROI参数，包括"成就"、"全部领取"、"战斗"等关键文本的识别区域
2. **坐标系统更新**：针对新版本界面调整了点击坐标的精确位置
3. **模板匹配优化**：更新了back.png等模板图片的匹配参数

### 自动战斗识别精度提升
系统通过以下机制提升自动战斗识别的准确性：

1. **多级识别验证**：采用OCR + 模板匹配的双重验证机制
2. **动态ROI调整**：根据界面变化自动调整识别区域
3. **识别结果过滤**：通过可信度阈值过滤不准确的识别结果

### 性能监控与自适应调节
系统实现了智能的性能监控和自适应调节机制：
...
**Diagram sources**
- [每日活动作战.json:132-137](file://assets/resource/base/pipeline/日常任务/每日活动作战.json#L132-L137)
- [reco_helper.py:208-231](file://agent/customs/maahelper/reco_helper.py#L208-L231)

**Section sources**
- [每日活动作战.json:1-504](file://assets/resource/base/pipeline/日常任务/每日活动作战.json#L1-L504)
- [reco_helper.py:62-95](file://agent/customs/maahelper/reco_helper.py#L62-L95)
...
**更新** 最新的每日活动作战管道变更进一步提升了系统的识别准确性和稳定性，通过ROI参数重算、坐标调整和智能识别验证机制，确保了在复杂界面变化下的可靠运行。

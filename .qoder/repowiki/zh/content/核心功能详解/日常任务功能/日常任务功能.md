...
- [回到主界面.json](file://MFAAvalonia/Resource/base/pipeline/通用/回到主界面.json)
- [懒加载返回主界面.json](file://MFAAvalonia/Resource/base/pipeline/通用/懒加载返回主界面.json)
</cite>

## 更新摘要
**所做更改**  
- **启动游戏识别策略重大升级**：移除了基于模板匹配的主界面识别节点，改用更稳定的多节点识别策略（活动弹窗、使徒、冒险通行证），通过`stable_reco`自定义识别器实现
- **OCR参数优化**：将游戏自动更新检测的OCR关键词从'开始下载'更新为'资源下载'，提升游戏自动更新检测的可靠性
- **视口配置增强**：启动游戏配置文件包含了完整的视口信息，支持界面缩放和定位
...
**重大更新** "启动游戏"是日常任务自动化流程的重要前置功能，负责启动游戏并进入可操作的主界面状态。该功能已进行重大改进，移除了基于模板匹配的主界面识别节点，改用更稳定的多节点识别策略。

启动游戏流程通过精确的OCR识别和模板匹配实现界面状态的自动判断，支持以下核心功能：
- **多节点识别策略**：通过`stable_reco`自定义识别器同时识别活动弹窗、使徒、冒险通行证三个关键节点
- **界面状态识别**：自动识别主界面、开始界面、公告界面等多种状态
- **弹窗处理**：自动处理公告、更新、奖励领取等弹窗
- **界面导航**：通过精确的视觉定位实现界面间的无缝导航
- **异常恢复**：支持多种异常情况的自动恢复机制

**更新** 启动游戏识别策略已从单一模板匹配升级为多节点识别：
- **稳定识别机制**：通过`stable_reco`自定义识别器实现界面状态的连续识别，减少误判概率
- **多节点并行识别**：同时识别活动弹窗、使徒、冒险通行证三个关键节点
- **识别阈值配置**：支持通过参数`m=5`设置连续识别次数阈值
- **节点列表支持**：支持传入多个节点名称进行并行识别

**更新** OCR参数检测逻辑已从'开始下载'更新为'资源下载'，提升了游戏自动更新检测的可靠性。

**本节来源**  
- [start_game.md](file://MFAAvalonia/Resource/descs/daily/start_game.md)
- [启动游戏.json:1-435](file://MFAAvalonia/Resource/base/pipeline/日常任务/启动游戏.json#L1-L435)
...
- [receive_reward.py:9-65](file://agent/customs/special_treat/receive_reward.py#L9-L65)
- [exam_cram.py:16-79](file://agent/customs/special_treat/exam_cram.py#L16-L79)
- [启动游戏.json:364-370](file://MFAAvalonia/Resource/base/pipeline/日常任务/启动游戏.json#L364-L370)
- [启动游戏.json:387-393](file://MFAAvalonia/Resource/base/pipeline/日常任务/启动游戏.json#L387-L393)
- [启动游戏.json:219-231](file://MFAAvalonia/Resource/base/pipeline/日常任务/启动游戏.json#L219-L231)
...
- **多节点识别机制**：通过`stable_reco`自定义识别器实现稳定的多节点识别
- **节点列表配置**：支持同时识别活动弹窗、使徒、冒险通行证三个关键节点
- **识别阈值设置**：通过参数`m=5`设置连续识别次数阈值
- **界面状态识别**：支持主界面、开始界面、公告界面等多种状态的自动识别
- **异常恢复节点**：配置了完整的JumpBack机制，支持多种异常情况的自动恢复
- **视觉定位优化**：通过精确的ROI区域和模板匹配实现高精度的界面元素定位
- **OCR关键词优化**：将'开始下载'更新为'资源下载'，提升更新检测的可靠性
- **视口配置**：配置了完整的视口信息，支持界面缩放和定位
...
- [启动游戏.json:364-393](file://MFAAvalonia/Resource/base/pipeline/日常任务/启动游戏.json#L364-L393)
...
- **稳定性保障**：通过精确的多节点识别机制和异常恢复机制，提高整个自动化流程的稳定性
...
- **多节点识别兼容性**：新的`stable_reco`自定义识别器需要确保与现有识别逻辑的兼容性
...
**重大更新章节** 基于应用变更，启动游戏功能得到了重大增强，特别是在界面状态识别和视觉定位方面：

### 多节点识别策略

启动游戏功能已移除了基于模板匹配的主界面识别节点，改用更稳定的多节点识别策略：

- **稳定识别器**：通过`stable_reco`自定义识别器实现界面状态的连续识别
- **多节点并行识别**：同时识别活动弹窗、使徒、冒险通行证三个关键节点
- **识别阈值配置**：通过参数`m=5`设置连续识别次数阈值，确保识别结果的稳定性
- **节点列表支持**：支持传入多个节点名称进行并行识别，提高识别速度
...
- **多节点识别机制**：通过`stable_reco`自定义识别器实现稳定的界面状态识别
...
- **节点列表并行处理**：支持同时识别活动弹窗、使徒、冒险通行证三个关键节点

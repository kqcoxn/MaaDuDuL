...
const PYTHON_EMBED_VERSION = project.maintenance.pythonRuntime.windows;
const PYTHON_STANDALONE_MINOR = project.python.recommendedPython;
...
function findPythonExecutableCandidate(binDir) {
    ...
    for (const name of [
        `python${PYTHON_STANDALONE_MINOR}`,
        `python${PYTHON_STANDALONE_MINOR}t`,
        "python",
    ]) {
...
async function resolvePythonStandaloneAsset(platform) {
    const response = await fetchGithubJson(
        `https://api.github.com/repos/astral-sh/python-build-standalone/releases/tags/${project.maintenance.pythonRuntime.standaloneRelease}`,
    );
    ...
    const pattern = new RegExp(
        `^cpython-${escapeRegExp(project.maintenance.pythonRuntime.standalone)}\\+${escapeRegExp(tag)}-${escapeRegExp(platformTriple)}-install_only_stripped\\.tar\\.gz$`,
    );

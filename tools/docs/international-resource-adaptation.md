...
- [3.3-ProjectInterfaceV2协议.md](D:/_Projects/MaaDuDuL/tools/docs/maafw-guide/3.3-ProjectInterfaceV2协议.md)
- [3.1-任务流水线协议.md](D:/_Projects/MaaDuDuL/tools/docs/maafw-guide/3.1-任务流水线协议.md)
- [4.2-标准化接口设计.md](D:/_Projects/MaaDuDuL/tools/docs/maafw-guide/4.2-标准化接口设计.md)

## Current Layout

This repository uses one shared task set and multiple resource paths.

- Base resource: `resource/base`
- China server resource: `resource/base`
- English / international overlay: `resource/en`
- Shared task definitions: `tasks`
- Chinese locale strings: `locales/interface_zh.json`
- English locale strings: `locales/interface_en.json`
...
- `en` loads the base resource first, then loads `resource/en` as an override layer.
...
If a node differs only in OCR text or a few fields, create a file with the same relative path under `resource/en`.

Example source file:

- base: `resource/base/pipeline/日常任务/领取邮件.json`

Example override file:

- en: `resource/en/pipeline/日常任务/领取邮件.json`
...
- `resource/base/pipeline/日常任务/启动游戏.json`
- `resource/base/pipeline/日常任务/关闭游戏.json`
- `interface.json` controller definitions, if applicable

If only the international resource differs, override the affected pipeline nodes in `resource/en/pipeline/...`.
...
1. add the Chinese file under `resource/descs/...`
2. add the English file under `resource/descs_en/...`
3. map the English locale key in `locales/interface_en.json`
...
3. If it is resource-level, add the smallest possible override under `resource/en`.
4. Keep the shared task layer unchanged unless there is no other clean option.
5. If you add new user-facing strings, update `locales/interface_en.json`.
...
- the change is placed in `resource/en` when it is server-specific
- the shared task layer is still shared unless duplication is truly required
- any new UI string has a locale entry
- any new English description file exists under `resource/descs_en`
...
- [resource/en/pipeline/日常任务/领取邮件.json](D:/_Projects/MaaDuDuL/resource/en/pipeline/日常任务/领取邮件.json:1)
